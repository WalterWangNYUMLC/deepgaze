...
class CnnHeadPoseEstimator:
    ...
    def load_yaw_variables(self, YawFilePath):
        ...
        if not os.path.isfile(YawFilePath):
            raise ValueError(
    ...
    def load_pitch_variables(self, pitchFilePath):
        ...
        if not os.path.isfile(pitchFilePath):
            raise ValueError('[DEEPGAZE] CnnHeadPoseEstimator(load_pitch_variables): the pitch file path is incorrect.')
...
class PnpHeadPoseEstimator:
    ...
    def __init__(self, cam_w, cam_h, dlib_shape_predictor_file_path):
        ...
        """
        if not IS_DLIB_INSTALLED:
            raise ValueError('[DEEPGAZE] PnpHeadPoseEstimator: the dlib libray is not installed. Please install dlib if you want to use the PnpHeadPoseEstimator class.')
        if not os.path.isfile(dlib_shape_predictor_file_path):
